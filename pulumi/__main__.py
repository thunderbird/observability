...
# Sometimes we want to update our local copy of Site24x7's obscure location data for our reference
...
for user_id, user_config in user_configs.items():
    # We have three special parameters to deal with: selection_type, statusiq_role, and user_role.
    # These must correlate to entries in our Enums, which correlate to enumerations on the API side.
    selection_type_name = user_config.pop('selection_type').upper()
    selection_type = (
        lib.SelectionType.__members__.get(selection_type_name, '') if selection_type_name else None
    )
...
def __create_monitors(all_monitor_groups, all_notification_profiles, all_user_groups):
    """
    Create the monitors defined in our configurations, resolving the names of monitoring and user
    groups into the IDs given to them by Site24x7.
    """
    ...
    for monitor_id, monitor_config in config.get_object('monitors').items():
        ...
        # Pop the "location_profile" option, replacing it with the correct name. This allows us to
        # change the name of a location profile without having to change every reference to it.
        ...
        # Convert monitor group names into their correct IDs, allowing us to refer to these by the
        # name we give them in the config and not the obscure numerical ID given by Site24x7's API.
...
# We need all monitor groups, notification groups, and user groups to be fully resolved so we can
# grab their API-assigned IDs. Here we collect all of these outputs and then create a compound
# output that we then resolve before actually creating these monitors.
monitor_group_ids = {
    monitor_group_id: monitor_group.id for monitor_group_id, monitor_group in monitor_groups.items()
}
...
user_group_ids = {user_group_id: user_group.id for user_group_id, user_group in user_groups.items()}
monitors = pulumi.Output.all(
